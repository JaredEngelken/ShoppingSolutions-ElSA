...
class GUI():
    def __init__(self, master):
        ...
        # Ad Buttons
##        self.adButton1 = tk.Button(self.bottomFrame1, text = 'Place ad here!', font = self.myFont, height = 96, width = 800,
##                                  command = lambda deal = 'Nuts' : self.discountDef(deal, 1.00, 1, self.adButton1))
##        self.photoNuts = tk.PhotoImage(file = "Planters.png")
##        self.adButton1.config(image = self.photoNuts)
        
        self.adButton2 = tk.Button(self.bottomFrame1, text = 'Place ad here!', font = self.myFont, height = 96, width = 800,
                                  command = lambda deal = 'Organic Milk' : self.discountDef(deal, 2.00, 2, self.adButton2))
        self.photoMilk = tk.PhotoImage(file = "HorizonAd640x106px.png")
        self.adButton2.config(image = self.photoMilk)
        self.adButton2.pack(fill = tk.BOTH)

##        self.buttons = ['adButton1', 'adButton2']#, self.adButton3, self.adButton4, self.adButton5]
##        for button in range(0, len(self.buttons)):
##            runButton = self.buttons[button]
##            print runButton
##            stripIt = runButton.strip(
##
##            runButton.pack(fill = tk.BOTH)
##            runButton.after(5000, runButton.destroy())
...
##        self.top.attributes("-topmost","-alpha",0.3, "true")
...
##            print row4search
            ...
            for cell in range(0, len(self.itemMaster)):
                ...
                def addCartList(thing1, thing2):
                    ...
                    for openslot in range(0, len(self.inCart)):
                        if self.inCart[openslot] == '':
                            self.inCart[openslot] = thing1
                            self.inCart.append('')
                    #print self.inCart

                    
                    shoppingCartButton = tk.Button(self.rightFrame1, text = thing1, bg = 'plum')
                    ...
                    TotalCost = round((sum(self.runningTotal[:-1]))+((sum(self.runningTotal[:-1]))*0.08845), 2)
...
##        addCartButton.pack(fill = tk.X)
        ...
        for tits in range(0, len(self.combinedMaster) - 1):
            ...
            self.addCartButton = tk.Button(self.leftFrame2, text = stringprinted, command = lambda thing1 = thing1: addCartList(thing1, thing2),
                                           bg = 'plum')
            self.addCartButton.pack(fill = tk.X)
            #print self.addCartButton.cget('text')

              # CART LIST
    def discountDef(self, deal, discount, minimum, buttonName):
        count = 0
        #print self.clickCheck
        for found in range(0, len(self.inCart)):
            if self.inCart[found] == deal:
                count = count+1
        if self.clickCheck == 0:
            if count >= minimum:
                for openslot in range(0, len(self.runningTotal)):
                    if self.runningTotal[openslot] == '':
                        discount = float(0-discount)
                        self.runningTotal[openslot] = round(discount, 2)
                        self.runningTotal.append('')
                #buttonName.destroy()
                self.clickCheck = 1
            TotalCost = round((sum(self.runningTotal[:-1]))+((sum(self.runningTotal[:-1]))*0.08845), 2)
            ...
            self.totalCostLabel.forget() 
            self.totalCostLabel = tk.Label(self.rightFrame1, text = "Total Cost: $"+str(TotalCost)+" ", bg = 'orchid', fg = 'lavenderblush',
                                           font = self.myFont)
            self.totalCostLabel.pack(fill = tk.X, side = tk.BOTTOM)
